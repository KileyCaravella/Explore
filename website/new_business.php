...
if (!$invalid_token) {
    ...
    $new_business = trim($_POST['new_business']);
    $category = trim($_POST['category']);

    $sql = "INSERT INTO user_accept (user_id, business_id, category_id) VALUES ('$token_user_id', '$new_business', '$category')";
    if (mysqli_query($con, $sql)) {

        //Once the business is added, the list of remaining businesses is returned to android
        if (ISSET($_POST['android']) && !$invalid_token) {
            $sql_get_categories = "SELECT business_id FROM user_accept WHERE (user_id = '$token_user_id' && category_id = '$category')";
